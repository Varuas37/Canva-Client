...
const SingleComment = ({
  ...
  const deleteComm = async () => {
    ...
    console.error("This is data from 104 line at CommentItem.js" + data._id);
...
function CommentItem({
  ...
  const press = (event) => {
    ...
    if (
      event.which == 8 ||
      (event.which == 46 && createComment.current.rows !== "1" && text !== "")
    ) {
      text = text.split("\n");
