...
function ExcelCheck({
	...
	useEffect(() => {
		uploadedFile ? (btnSubmit.current.disabled = false) : (btnSubmit.current.disabled = true);
	}, [uploadedFile]);

	useEffect(() => {
		favouriteCourse.length > 0 ? console.log('Already Fetched Courses') : getFavCourses();
	});
	...
	const onSubmit = async (data) => {
		...
		setTimeout(async () => {
			await assignExcelFile(uploadedFile.name, excelPassword, selectedCourse,assignmentID);
		}, 4000);
		setTimeout(() => {
			history.push('/');
		}, 6000);
								...
								<div className="space-y-6 sm:space-y-5">
									<div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
										...
										{errors.selectedCourse && (
											<p className="sm:text-sm text-red-500">Please select a class</p>
										)}
									</div>
									...
									<div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
										...
										{errors.assignmentID && (
											<span className="sm:text-sm text-red-500">Please type assignment ID</span>
										)}
									</div>
									...
									<div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
										...
										{errors.excelPassword && (
											<span className="sm:text-sm text-red-500">This field is required</span>
										)}
									</div>
								</div>
							</div>
