     ...
     (
      <Fragment>
        ...
        <li class="px-6 py-5 relative">
          <div
            class="group flex justify-between items-center space-x-2"
              ...
              <div class="flex-1 flex items-center min-w-0 relative">
                ...
                <div class="ml-4 truncate">
                  <div class="text-sm leading-5 font-medium text-gray-900 truncate">
                    {title?title:"Error Loading Title"}
                  </div>
                  <div class="text-sm leading-5 text-gray-500 truncate">
                    Due :{" "}
                    <Moment fromNow>{dueDate ? dueDate : "No Due Date"}</Moment>{" "}
                    (<Moment format="dddd, MMMM DD">{dueDate}</Moment>)
                  </div>
                </div>
              </div>
            </a>
        ...
        <div className={`px-4 py-5 sm:p-6 ${showDetails ? "hidden" : null}`}>
          <div class="mt-2 max-w-xl text-sm text-gray-500">
            <p>{description ? parse(description):"Description not found"}</p>
          </div>
        </div>
      </Fragment>
    )
